...
pub fn getNextSqrtPriceFromAmount0RoundingUp(
    ...
    if add {
        ...
        if product / amount == sqrtPX96.into() {
            let denominator: U256 = numerator1 + product;
            if denominator >= numerator1 {
                // always fits in 160 bits
                return full_math::mulDivRoundingUp(numerator1, sqrtPX96.into(), denominator)?.try_into();
            }
        }

        return (full_math::unsafeDivRoundingUp(numerator1, (numerator1 / sqrtPX96).add(amount)))?.try_into();
    } else {
        // if the product overflows, we know the denominator underflows
        // in addition, we must check that the denominator does not underflow
        let product: U256 = amount * sqrtPX96;
        ensure!(
            product / amount == sqrtPX96.into() && numerator1 > product,
...
fn getNextSqrtPriceFromAmount1RoundingDown(
    ...
    if add {
        ...
        (U256::from(sqrtPX96) + quotient).try_into()
    } else {
        let quotient: U256 = if amount <= U160::MAX.into() {
        ...
        ensure!(U256::from(sqrtPX96) > quotient, "fdsajkfldsaw TODO");
        // always fits 160 bits
        Ok(sqrtPX96 - quotient.try_into()?)
    }
}
...
pub fn getNextSqrtPriceFromInput(
    ...
    ensure!(sqrtPX96 > U160::zero(), "zero price");
...
pub fn getAmount0DeltaHelper(
    ...
    let numerator2: U256 = (sqrtRatioBX96 - sqrtRatioAX96).into();

    ensure!(sqrtRatioAX96 > U160::zero(), "price zero");

    return if roundUp {
        full_math::unsafeDivRoundingUp(
            full_math::mulDivRoundingUp(numerator1, numerator2, sqrtRatioBX96.into())?,
            sqrtRatioAX96.into(),
        )
    } else {
        Ok(full_math::muldiv(numerator1, numerator2, sqrtRatioBX96.into())? / sqrtRatioAX96)
    };
}
...
pub fn getAmount1DeltaHelper(
    ...
    if roundUp {
        full_math::mulDivRoundingUp(
            liquidity.into(),
            (sqrtRatioBX96 - sqrtRatioAX96).into(),
            *fixed_point::q96,
        )
    } else {
        full_math::muldiv(
...
pub fn getAmount0Delta(sqrtRatioAX96: U160, sqrtRatioBX96: U160, liquidity: i128) -> Result<I256> {
    Ok(if liquidity < 0 {
...
pub fn getAmount1Delta(sqrtRatioAX96: U160, sqrtRatioBX96: U160, liquidity: i128) -> Result<I256> {
    Ok(if liquidity < 0 {
